...
ch_ret move_char( CHAR_DATA *ch, EXIT_DATA *pexit, int fall )
{
    ...
    if( ch->cover != COVER_NONE )
     {
	send_to_char("Not while still ducked and covered.&w\n\r", ch );
	return rNONE;
     }

    if( ch->cards != CARD_NONE )
     {
	send_to_char( "&rYou must fold your hand before you can leave.&w\n\r", ch );
	return rNONE;
     }
...
*/
	...
	if ( IS_SET( pexit->exit_info, EX_CLIMB ) )
	{
	    ...
	    if ( !found && !ch->mount )
	    {
		diff = ( number_range( 10, 25 ) );
		if ( ( !IS_NPC(ch) && diff > (number_range(1, 20) + ch->skill_level[CLIMB_SKILL] + stat_table[get_curr_str(ch)].mod) )
	...
	if ( ch->mount )
	{
	  switch (ch->mount->position)
	  {
	    ...
	    case POS_MEDITATING:
	    send_to_char( "Your mount is currently meditating?\n\r", ch );
	    return rNONE;
	    break;
    ...
    && ( IS_NPC(ch) || !IS_SET(ch->act, PLR_WIZINVIS) ) )
    {
  	  ...
  	  else
	  {
	    if ( drunk )
	     ...
	     {
		if(ch->race == RACE_DROID && ch->droid_type == DROID_SECOND)
		  txt = "rolls";
		else
  	          txt = "leaves";
	     }
    ...
    if( IS_SET(pexit->exit_info, EX_AUTOCLOSE) && IS_SET(pexit->exit_info, EX_ISDOOR) )
     {
      int dir;
      EXIT_DATA *oexit;
	...
	dir = DIR_SOUTH;
      else if( pexit->vdir == DIR_EAST )
	dir = DIR_WEST;
      else if( pexit->vdir == DIR_SOUTH )
	dir = DIR_NORTH;
      else if( pexit->vdir == DIR_WEST )
	dir = DIR_EAST;
      else if( pexit->vdir == DIR_NORTHEAST )
	dir = DIR_SOUTHWEST;
      else if( pexit->vdir == DIR_SOUTHEAST )
	dir = DIR_NORTHWEST;
      else if( pexit->vdir == DIR_SOUTHWEST )
	dir = DIR_NORTHEAST;
      else if( pexit->vdir == DIR_NORTHWEST )
	dir = DIR_SOUTHEAST;
      if( ( oexit = get_exit( to_room, dir ) ) != NULL )
       {
	if( !IS_SET(oexit->exit_info, EX_ISDOOR) )
     	 SET_BIT(oexit->exit_info, EX_ISDOOR);
	if( !IS_SET(oexit->exit_info, EX_CLOSED) )
	 SET_BIT(oexit->exit_info, EX_CLOSED);
	if( !IS_SET(oexit->exit_info, EX_AUTOCLOSE) )
	 SET_BIT(oexit->exit_info, EX_AUTOCLOSE);
       }
      act( AT_BLUE, "A door automatically closes.", ch, NULL, NULL, TO_ROOM );
      act( AT_BLUE, "A door automatically closes behind you.", ch, NULL, NULL, TO_CHAR );
      send_to_char( "\n\r", ch );
    }
...
void do_up( CHAR_DATA *ch, char *argument )
{
    ...
    if( ( exit = get_exit( ch->in_room, DIR_UP ) ) != NULL && IS_SET( exit->exit_info, EX_xCLIMB ) )
    {
        send_to_char( "That looks too steep to walk up, you must climb up it.\n\r", ch );
	return;
    }    
...
void do_down( CHAR_DATA *ch, char *argument )
{
    ...
    if( ( exit = get_exit( ch->in_room, DIR_DOWN ) ) != NULL && IS_SET( exit->exit_info, EX_xCLIMB ) )
    {
	send_to_char("That appears too steep to walk down, you will have to climb.\n\r", ch );
	return;
    }
...
void do_open( CHAR_DATA *ch, char *argument )
{
	...
	{
	  send_to_char( "&rUse lockercode to unlock lockers.&w\n\r", ch );
	  return;
	}
...
void do_close( CHAR_DATA *ch, char *argument )
{
    ...
    if( ( obj = get_obj_list( ch, arg, ch->in_room->first_content ) ) != NULL )
    {
	if( obj->item_type == ITEM_LOCKER )
	 {
	  if( IS_SET(obj->value[0], CONT_CLOSED) )
	   {
		send_to_char( "&GThat locker is already closed.&w\n\r", ch );
		return;
	   }
	  SET_BIT(obj->value[0], CONT_CLOSED);
	  SET_BIT(obj->value[0], CONT_LOCKED);
          act( AT_ACTION, "You hear a faint click as you close $p.", ch, obj, NULL, TO_CHAR );
          act( AT_ACTION, "You hear a faint click as $n closes $p.", ch, obj, NULL, TO_ROOM );
	  return;
	 }
    }
    if ( ( pexit = find_door( ch, arg, TRUE ) ) != NULL )
...
void do_bashdoor( CHAR_DATA *ch, char *argument )
{
	...
	if ( ( pexit = find_door( ch, arg, FALSE ) ) != NULL )
	{
	    ...
	    if ( !IS_NPC(ch) )
		chance = (number_range(1,20) + stat_table[get_curr_str(ch)].mod);
	    else
		chance = 90;
		...
		damage( ch, ch, ( ch->max_hit / 20 ), TYPE_HIT, FALSE );
		...
		damage( ch, ch, ( ch->max_hit / 20 ) + 10, TYPE_HIT, FALSE );
	    }
	}    
	else
	{
...
void do_stand( CHAR_DATA *ch, char *argument )
{
	...
	act( AT_ACTION, "$n rises from $s meditation.", ch, NULL, NULL, TO_ROOM );
	ch->position = POS_STANDING;
	break;
...
void do_sit( CHAR_DATA *ch, char *argument )
{
    ...
    case POS_MEDITATING:
	send_to_char( "You stop meditating and sit.\n\r", ch );
	act( AT_ACTION, "$n stops meditating and sits up.", ch, NULL, NULL, TO_ROOM );
	ch->position = POS_SITTING;
	break;
...
void do_rest( CHAR_DATA *ch, char *argument )
{
    ...
    case POS_MEDITATING:
	send_to_char( "You relax from your meditations and sprawl out on the ground.\n\r", ch );
	act( AT_ACTION, "$n relaxes from $s meditations and sprawls out on the ground.", ch, NULL, NULL, TO_ROOM );
	ch->position = POS_RESTING;
	break;
...
void do_sleep( CHAR_DATA *ch, char *argument )
{
    ...
    case POS_MEDITATING:
	send_to_char( "You stop meditating and lay down for a nice sleep.\n\r", ch );
	act( AT_ACTION, "$n stops meditating and lays down for a relaxing sleep.", ch, NULL, NULL, TO_ROOM );
...
void do_duck( CHAR_DATA *ch, char *argument )
{

  if( IS_NPC(ch) )
    return;

  if( ch->cover != COVER_NONE && ch->cover != COVER_DUCK )
   {
	send_to_char("&wYou're already covered, why would you want to duck?\n\r", ch );
	return;
   }
  if( ch->cover == COVER_DUCK )
   {
	ch->cover = COVER_NONE;
	send_to_char("&YYou stand up.&w\n\r", ch );
        act( AT_YELLOW, "%s stands up.&w\n\r", ch, NULL, NULL, TO_NOTVICT );
	WAIT_STATE( ch, PULSE_VIOLENCE );
	return;
   }
  ch->cover = COVER_DUCK;
  send_to_char("&YYou duck down, making yourself a smaller target.&w\n\r", ch );	 
  act( AT_YELLOW, "$n ducks down, decreasing their size.&w\n\r", ch, NULL, NULL, TO_NOTVICT );
  WAIT_STATE( ch, PULSE_VIOLENCE );
  return;
}

void do_lean( CHAR_DATA *ch, char *argument )
{
  CHAR_DATA *gch;
  ...
  if( argument[0] == '\0' )
   {
	send_to_char("&wLean which way? Left or Right?&w\n\r", ch );
	return;
   }
  if( !str_cmp(argument, "left") )
   {
	if( ch->cover == COVER_LEFT )
	 {
		send_to_char( "&rYou are already leaning to the left!&w\n\r", ch );
		return;
	 }
	if( ch->cover == COVER_RIGHT )
	 {
	 	send_to_char( "&YYou lean back towards the left, hiding most of your body.&w\n\r", ch );
		for( gch = ch->in_room->first_person; gch; gch = gch->next_in_room )
	 	  ch_printf( gch, "&Y%s leans back to the left, disappearing mostly from sight.&w\n\r", ch->name );
		ch->cover = COVER_FULL;
		WAIT_STATE( ch, PULSE_VIOLENCE );
		return;
	 }
	if( ch->cover == COVER_FULL )
	 {
		send_to_char( "&YYou lean towards the left, exposing portions of your body.&w\n\r", ch );
	 	for( gch = ch->in_room->first_person; gch; gch = gch->next_in_room )
		  ch_printf( gch, "&Y%s leans out to the left, exposing portion of their body.&w\n\r", ch->name );
		ch->cover = COVER_LEFT;
	  	WAIT_STATE( ch, PULSE_VIOLENCE );
		return;
	 }
	...
	if( ch->cover == COVER_RIGHT )
	 {
		send_to_char("&RYou are already leaning to the right, anymore and you'll lose your cover.&w\n\r", ch );
		return;
	 }
	if( ch->cover == COVER_FULL )
	 {
		send_to_char("&YYou lean out to the right, explosing portions of your body.&w\n\r", ch );
		for( gch = ch->in_room->first_person; gch; gch = gch->next_in_room )
		  ch_printf( gch, "&Y%s leans out to the right, exposing part of their body.&w\n\r", ch->name );
		ch->cover = COVER_RIGHT;
		WAIT_STATE(ch, PULSE_VIOLENCE );
		return;
 	 }
	if( ch->cover == COVER_LEFT )
	 {
		send_to_char("&YYou lean back to the right, covering most of your body.&w\n\r", ch );
		for( gch = ch->in_room->first_person; gch; gch = gch->next_in_room )
		  ch_printf( gch, "&Y%s leans back to the right, covering most of their body.&w\n\r", ch->name );
		ch->cover = COVER_FULL;
		WAIT_STATE(ch, PULSE_VIOLENCE);
		return;
	 }
  ...
  if( argument[0] == '\0' )
   {
    	if( ch->cover == COVER_NONE )
	 {
		send_to_char( "&wTake cover behind what?\n\r", ch );
		return;
	 }
	ch->cover = COVER_NONE;
	send_to_char("&YYou come out from behind your cover.&w\n\r", ch );
	for( gch = ch->in_room->first_person; gch; gch = gch->next_in_room )
	 ch_printf( gch, "&Y%s comes out from behind cover.&w\n\r", ch->name );
	WAIT_STATE(ch, PULSE_VIOLENCE);
	return;
   }
  if( ch->position != POS_STANDING )
   {
	send_to_char("&wYou can't do this in your present state.\n\r", ch );  
	return;
   }
  if( ch->cover != COVER_NONE && ch->cover != COVER_DUCK )
   {
	send_to_char("&wYou're already under cover.\n\r", ch );
	return;
   }
  ...
  if( obj )
   {
    if( obj->item_type != ITEM_FURNITURE )
     {
	send_to_char("&wHow are you going to take cover behind that?\n\r", ch );
	return;
     }
    if( IS_SET(obj->pIndexData->extra_flags, ITEM_TAKE) )
     {
	send_to_char("&wYou can't take cover behind objects people make pick up.&w\n\r", ch );
	return;
     }
    ch->cover = COVER_FULL;
    ch_printf( ch, "&YYou dive and take cover behind %s.&w\n\r", obj->short_descr );
    WAIT_STATE( ch, PULSE_VIOLENCE );
    for( gch = ch->in_room->first_person; gch; gch = gch->next_in_room )
     {
	if( gch == ch )
	  continue;
	ch_printf( gch, "&Y%s dives and takes cover behind %s.&w\n\r", ch->name, obj->short_descr );
     }
    return;
   }
  if( ship )
   {
	...
	if( gch == ch )
	  continue;
	ch_printf( gch, "&Y%s dives and covers behind %s.&w\n\r", ch->name, ship->name );
     }
    return;
   }
  send_to_char( "&wThat doesn't seem to be here to take cover behind.\n\r", ch );
  return;
}
